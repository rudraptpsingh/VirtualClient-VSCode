...
export function isTestEnvironment(): boolean {
    return TEST_ENV_VARS.some(
        envVar => process.env[envVar] === 'test' || process.env[envVar] === '1' || process.env[envVar] === 'true'
    );
}
...
export async function showConfirmationDialog(
    ...
    return await vscode.window.showWarningMessage(message, { modal }, confirmLabel);
}

/**
 * Standardized error message display
 */
export function showErrorMessage(error: Error | string, context?: string): void {
 ...
 */
export async function showWarningMessage(message: string, ...actions: string[]): Promise<string | undefined> {
 ...
 */
export function getExtensionConfig<T>(section: string, key: string, defaultValue: T): T {
