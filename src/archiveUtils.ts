 ...
 */
export async function executeArchiveCommand(conn: Client, archiveCommand: string, logger?: any): Promise<void> {
 ...
 */
export async function extractTarArchive(localArchivePath: string, localLogsDir: string, logger?: any): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        exec(tarCmd, (error: any, stdout: string, stderr: string) => {
            if (error) {
                logger?.error(`Error extracting logs.tar.gz: ${stderr}`);
